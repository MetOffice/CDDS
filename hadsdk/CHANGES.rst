...
* Fixed an import bug in the ``mip_table_editor`` script (#2463)

Release 2.1.1, October 26, 2021
===============================

* Improved error logging for the write_rose_suite_request_json script (#2390)
...
* Inventory can now be constructed or updated from CREPP database dump on JASMIN (#2148)
* Corrected masking of polar rows for the ORCA12 grid (#2211)
* Implemented changes necessary for conda deployment of CDDS code on JASMIN (#2240)

Release 2.0.1, March 25, 2021
=============================

* ``write_rose_suite_request_json`` can now create request files for the UKESM1-ice-LL
  model (#2264)
* ``write_rose_suite_request_json`` now allows the `apt` stream (#2283)
...
* ``write_rose_suite_request_json`` now correctly handles fields that are
...
* Errors in rose suite metadata (rose-suite.info file) now cause
  write_rose_suite_request_json to fail rather than warn and branch
  dates are correctly handled (#1477)
* Users can now override the start and end dates for processing when creating
...
* Implemented a new command line tool ``search_inventory`` for exploring
  archived datasets and checking their publication status (#1903)
* Implemented code changes needed to support ARCHER filepath format
...
* Institution names and branching dates are now correctly processed and
...
* Added ``write_rose_suite_request_json`` to construct a request JSON file
  from the rose-suite.info file within a CMIP6 suite (#1732)

Release 1.4.2, April 30, 2020
=============================

* ``write_request_json`` now gives a useful error message when
...
* When writing request JSON files from CREM, warnings are issued if
...
* ``thetaot`` and ``sltbasin`` can now be produced after correcting their
  stream (#1009)
...
* PP header fixes are now correctly applied when producing information
...
* The ``mip_era`` and ``--root_config`` command line options for the
...
* The value of the ``parent_time_units`` attribute can now be interpreted by
  ``cf_units`` (#1220)
* The ``write_request_json`` script now writes the ``parent_experiment_id`` to
  the request JSON file (#1109)

Release 1.1.4, September 2, 2019
================================

* ``Experiment name not found in this version of the data request`` errors in
...
* Ocean ancillary variables are now correctly recognised in several
...
* The production of MEDUSA ocean biogeochemistry |MIP requested variables| are
  now supported (#582)
* HadSDK now preferentially uses ``cftime``, falling back to using
...
* The ``hadsdk.common.netCDF_regexp()`` function now returns a regular
...
* The ``write_request_json`` script now works correctly for experiments without
