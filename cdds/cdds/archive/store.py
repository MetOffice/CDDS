...
def store_mip_output_data(request: Request, stream: str, mip_approved_variables_file: str) -> int:
    ...
    ----------
    request : Request
        Request object containing information to store netCDF files
    stream : str
        The |Stream identifier| of the |MIP output variables| to be processed. If none, all variables will be processed.
    mip_approved_variables_file : str
...
def get_variables_to_process(
    ...
    ----------
    request : Request
        Request containing all information to get variables for processing
    mip_approved_variables_file : str
        The path to approved variables file output by CDDS quality control.
    selected_stream : str
        The |Stream identifier| of the |MIP output variables| to be processed. If none, all variables will be processed.
...
def get_approved_variables(
    ...
    ----------
    request : Request
        Request containing all information to get the approved variables
    mip_approved_variables_path : str
        (Optional) Path to the approved variables file defined which variable is approved. If none is given the function
        will search for the QC generated approved variable file.
    selected_stream : str
        (Optional)
...
def get_active_variables(request_vars_path: str) -> List[Dict[str, str]]:
    ...
    -------
    List[Dict[str, str]]
        A list of dictionaries for each active variable, each dictionary containing all the information specific to one
...
def read_approved_vars_from_file(mip_approved_variables_path: str) -> List[Dict[str, str]]:
    ...
    -------
    List[Dict[str, str]]
        A list of dictionaries for each approved variable, each dictionary containing all the information specific to
        one |MIP output variable| required to archive the relevant |output netCDF files|.
    """
