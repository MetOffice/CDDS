...
def repack_single_file(nc_file: Path) -> bool:
    """
    Check and repack a single NetCDF file if needed, then verify the result.

    If the file does not need repacking, returns immediately. If it does,
    repacks it and verifies the output passes the packing check. Raises a
    RuntimeError if the repacked file still fails verification.
    ...
    ------
    RuntimeError
        If the file fails the packing check after repacking.
    """
    if run_check_cmip7_packing(str(nc_file)) == 0:
        return False
    run_cmip7repack(str(nc_file))
    if run_check_cmip7_packing(str(nc_file)) != 0:
        raise RuntimeError(f"Packing verification failed after repack: '{nc_file}'")
    return True
