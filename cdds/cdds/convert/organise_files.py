...
def transpose_directory_structure(mip_convert_output_dir, staging_dir,
    ...
    start_date:  str
        Start date for the window of directories and files to be moved.
    end_date:  str
        End date for the window of directories and files to be moved.
...
def identify_files_to_move(location, start_date, end_date):
    ...
    start_date: str
        Start date for processing window. Window is inclusive of start date.
    end_date: str
        End date for processing window. Window is inclusive of end date.
    ...
    for cycle_dir in sorted(os.listdir(location)):
        if re.match(r'\d{4}-\d\d-\d\d', cycle_dir):
            ...
            if start_dt <= datestamp <= end_dt:
                location_cycle = os.path.join(location, cycle_dir)
                logger.info('path {0} included in this concatenation window'.format(location_cycle))
                ...
                for component in os.listdir(location_cycle):
                    ...
                    for filename in glob.iglob(path_to_input_files):
                        cmorvar, miptable = os.path.basename(filename).split('_')[:2]
    ...
    for miptable in sorted(files_to_move):
        msg = []
        for cmorvar in files_to_move[miptable]:
            msg.append('{} ({})'.format(cmorvar, len(files_to_move[miptable][cmorvar])))
        logger.info('mip table "{}" : {}'.format(miptable, ', '.join(msg)))

    logger.info('The following files to move have been identified:')
    log_msg_files_to_move = pprint.pformat({k1: dict(v1) for k1, v1 in files_to_move.items()}, width=1)
...
def organise_files():
    ...
    Calendar.default().set_mode(params['CALENDAR'])
    ...
    config_params = {
        ...
        'reference_date': params['REF_DATE'],
