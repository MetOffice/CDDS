...
def main_checkout_workflow(arguments: Union[list, None] = None):
    ...
    Parameters
    ----------
    arguments : Union[list, None], optional
        Optionally pass in a list of arguments for testing purposes.

    Raises
    ------
    FileNotFoundError
        Raised if the target directory doesn't exist.
    """
...
def update_rose_conf(args, workflow_dst: Path):
    ...
    args : argparse.Namespace
        User arguments from command line
    workflow_dst : Path
        Workflow destination
    """
...
def get_request_files(request_location: str) -> list:
    ...
    request_location : str
        Path to request file(s)

    Raises
    ------
    IOError
        If request_location is invalid.
    IOError
