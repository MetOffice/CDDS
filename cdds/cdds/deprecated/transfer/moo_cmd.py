...
def put(local, remote, simulation=False):
    ...
    ----------
    local: str
        path to file(s) to put (can include wildcard)
    remote: str
        MASS destination directory
    simulation: bool
        if true simulate moo command.
    """
    arg = local + [remote]
...
def get(remote, local, transfer_threads, simulation=False, logger=None):
    ...
    ----------
    remote: str
        path to file(s) on MASS to copy (can include wildcards).
    local: str
        path to local destination directory.
    transfer_threads: str
        number of transfer threads to use to copy files.
    simulation: bool
        if true simulate moo command.
    logger: logging.Logger
        Logger to use. If omitted a logger will be obtained within :func:`cdds.deprecated.transfer.moo.run_moo_cmd`.
    """
...
def ls(moose_dir, simulation=False, logger=None):
    ...
    ---------
    moose_dir: str
        path to MASS directory to list
    simulation: bool
        if true simulate moo command.
    logger: logging.Logger
        Logger to use. If omitted a logger will be obtained within :func:`cdds.deprecated.transfer.moo.run_moo_cmd`
...
def ls_tree(moose_dir, simulation=False):
    ...
    ----------
    moose_dir: str
        path to top loevel directory to list.
    simulation: bool
        if true simulate moo command.
    """
    if simulation == LS_ONLY:
...
def mv(old, new, simulation=False):
    ...
    ----------
    old: str
        path to original MASS path
    new: str
        path to new MASS path
    simulation: bool
        if true simulate moo command.
    """
