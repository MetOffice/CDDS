...
class TestOrganiseFxOutputDir(unittest.TestCase):
    """Tests for organise_fx_output_dir."""
    ...
    @mock.patch('os.path.isdir')
    @mock.patch('os.listdir')
    def test_organise_fx_output_dir(self, mock_listdir, mock_isdir, mock_makedirs, mock_rmdir, mock_move):
        """Files are moved from component dirs into mip_table/variable dirs."""
        output_dir = '/output/afx'
        mock_listdir.side_effect = [
            ['atmos-native'],
            ['orog_fx_MODEL_piControl_r1i1p1f2_gn.nc',
             'sftlf_fx_MODEL_piControl_r1i1p1f2_gn.nc',
             'create_cdds_directory_structure.log'],
        ]
        mock_isdir.side_effect = lambda p: p in [
            '/output/afx/atmos-native',
        ]

        cdds.convert.organise_files.organise_fx_output_dir(output_dir)

        mock_makedirs.assert_any_call('/output/afx/fx/orog')
        mock_makedirs.assert_any_call('/output/afx/fx/sftlf')
        mock_move.assert_any_call(
            '/output/afx/atmos-native/orog_fx_MODEL_piControl_r1i1p1f2_gn.nc',
            '/output/afx/fx/orog')
        mock_move.assert_any_call(
            '/output/afx/atmos-native/sftlf_fx_MODEL_piControl_r1i1p1f2_gn.nc',
            '/output/afx/fx/sftlf')
    ...
    @mock.patch('os.path.isdir')
    @mock.patch('os.listdir')
    def test_organise_fx_output_dir_ofx(self, mock_listdir, mock_isdir, mock_makedirs, mock_rmdir, mock_move):
        """Ocean fx files are moved into Ofx/variable dirs."""
        output_dir = '/output/ofx'
        mock_listdir.side_effect = [
            ['ocean-native'],
            ['areacello_Ofx_MODEL_piControl_r1i1p1f2_gn.nc',
             'deptho_Ofx_MODEL_piControl_r1i1p1f2_gn.nc'],
        ]
        mock_isdir.side_effect = lambda p: p in [
            '/output/ofx/ocean-native',
        ]

        cdds.convert.organise_files.organise_fx_output_dir(output_dir)

        mock_makedirs.assert_any_call('/output/ofx/Ofx/areacello')
        mock_makedirs.assert_any_call('/output/ofx/Ofx/deptho')
        mock_move.assert_any_call(
            '/output/ofx/ocean-native/areacello_Ofx_MODEL_piControl_r1i1p1f2_gn.nc',
            '/output/ofx/Ofx/areacello')
        mock_move.assert_any_call(
            '/output/ofx/ocean-native/deptho_Ofx_MODEL_piControl_r1i1p1f2_gn.nc',
            '/output/ofx/Ofx/deptho')
