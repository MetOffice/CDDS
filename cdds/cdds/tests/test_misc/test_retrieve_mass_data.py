...
class TestTransferFiles(unittest.TestCase):
    ...
    @patch("cdds.misc.retrieve_mass_data.logger")
    def test_transfer_files_dry_run(self, mock_logger, mock_run_mass_command):
        mock_run_mass_command.return_value = ("#### get, command-id=2059286267, "
        ...
        for call in mock_logger.info.call_args_list:
            self.assertIn(("#### get, command-id=2059286267, "
            "estimated-cost=60722658byte(s), files=1, media=0"), call[0][0])

    @patch("cdds.misc.retrieve_mass_data.run_mass_command", side_effect=RuntimeError("fail"))
    @patch("cdds.misc.retrieve_mass_data.logger")
    def test_transfer_files_runtime_error(self, mock_logger, mock_run_mass_command):
