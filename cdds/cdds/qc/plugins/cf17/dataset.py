...
class Cf17Dataset(StructuredDataset):
    ...
    def __init__(self, root, request, mip_tables, mip_table=None, start=None, end=None, logger=None, stream=None):
        super(Cf17Dataset, self).__init__(root, request, mip_tables, mip_table, start, end, logger, stream)
    ...
    def check_filename(self, ds, filename):
        ...
        if len(member_id) > 1:
            ...
            try:
                if self.getncattr("sub_experiment_id", ds) == "none":
                    messages.append(
            ...
            sub_exp_id = self.getncattr("sub_experiment_id", ds, True)
        ...
        if re.match(r"^r\d+i\d+p\d+f\d+$", label_candidate) is None:
            ...
            variant_label = self.getncattr("variant_label", ds)
    ...
    def _aggregate_files(self):
        ...
        for filepath in self._dataset:
            with self._loader_class(filepath) as ds:
                ...
                try:
                    file_index = '_'.join([self.getncattr(x, ds) for x in attrs])
                    try:
                        var_name = self.getncattr('variable_name', ds)
                    except AttributeError:
                        ...
                        var_name = self.getncattr('variable_id', ds)
