...
class DatetimeCalculator():
    ...
    def days_since_base_date(self, time_point: str) -> float:
        return (parse.TimePointParser().parse(time_point) - self.base_date)._get_non_nominal_seconds() / self.seconds_in_day
    ...
    def get_sequence(self, start_date: str, end_date: str, mode: str):
        ...
        sequence_bounds = []
        while current < end:
            timepoint_tuple = self._get_bounds_and_midpoint(current, duration)
    ...
    def _get_bounds_and_midpoint(self, current_time_point, duration):
