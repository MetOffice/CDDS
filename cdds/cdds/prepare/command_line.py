...
def main_generate_variable_list(arguments: List[str] = None) -> int:
    ...
    load_mapping_plugin(
        request.conversion.mip_convert_plugin,
...
def parse_generate_args(arguments: List[str]) -> Namespace:
    ...
    parser.add_argument(
        '--remove-orphaned-tasks', action='store_true', help=(
            'Reinstall and reload an already running conversion workflow so that tasks for components '
            'that no longer have any variables are removed. This implies --reconfigure.')
    )
