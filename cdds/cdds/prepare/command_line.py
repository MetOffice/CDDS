...
def parse_generate_args(arguments: List[str]) -> Namespace:
    ...
    parser.add_argument(
        '--plugin_module_path', help=(
            'If you use a external plugin: Path to the module that contains the implementation of the plugin '
            'that should be loaded'
        )
    )
    parser.add_argument(
        '--plugin_location', help=(
            'If you use a external plugin: Path to the plugin implementation that should be added to the '
            'PYTHONPATH'
        )
    )
    ...
    load_mapping_plugin(args.plugin_id, args.plugin_module_path, args.plugin_location)
