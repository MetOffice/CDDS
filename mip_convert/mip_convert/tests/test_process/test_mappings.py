...
PATH_TO_PROCESS = os.path.join(
    os.path.dirname(os.path.realpath(mip_convert.__file__)), 'process')
CFG_SUFFIX = '_mappings.cfg'
CFG_FILES = sorted([filename for filename in os.listdir(PATH_TO_PROCESS)
                    if filename.endswith(CFG_SUFFIX)])
...
@attr('mappings')
def test_required_options_in_configuration_files():
    for (cfg_file, model_id, mip_table_id) in test_info():
        cfg_path = os.path.join(PATH_TO_PROCESS, cfg_file)
        sections_to_test = ModelToMIPMappingConfig(cfg_path, model_id).sections
        model_to_mip_mappings = get_model_to_mip_mappings(
            model_id, 'CMIP6_{}'.format(mip_table_id))
        for section in sections_to_test:
            if section not in ['COMMON']:
                mappings = Mappings(section, mip_table_id, cfg_file,
                                    model_to_mip_mappings)
                yield (check_model_to_mip_mappings, mappings)
