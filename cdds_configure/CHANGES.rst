...
* `generate_user_config_files` is now run by `cdds_convert` by default (#1902, #2425)
...
* Settings and files needed for production of CF sites data are now
...
* The ``cdds_configure`` script is now called ``generate_user_config_files``
  (#1043)
* The ``--mohc`` and ``--root_config`` command line options for the
...
* The ``--data_request_version``, ``--template_name``, ``--root_proc_dir`` and
...
* MIP Convert template files for the ocean are now produced separately for each
...
* The grid label ``gn`` is now used for datasets that use |model output files|
